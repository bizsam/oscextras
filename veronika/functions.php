<?php
define('VERONIKA_THEME_VERSION', '1.6.3');
...
function veronika_theme_info() {
  return array(
    ...
    'version'   => '1.6.3',
    'description' => 'Powerful theme for classifieds',
    'author_name' => 'MB Themes',
    'author_url'  => 'https://osclasspoint.com',
    'support_uri'  => 'https://forums.osclasspoint.com/veronika-osclass-responsive-theme/',
...
// THUMBNAIL URL FOR LAZYLOAD
function veronika_resource_thumbnail_url($img_url = '') {
  if(osc_get_preference('force_aspect_image', 'osclass') <> 1 && osc_get_preference('lazyload', 'veronika_theme') == 1) {
    return osc_current_web_theme_url('images/no-image.png');
  } else {
    if($img_url <> '') {
      return $url;
    } else {
      return osc_resource_thumbnail_url();
    }
  }
}
...
// RESOURCE URL FOR LAZYLOAD
function veronika_resource_url($img_url = '') {
  if(osc_get_preference('force_aspect_image', 'osclass') <> 1 && osc_get_preference('lazyload', 'veronika_theme') == 1) {
    return osc_current_web_theme_url('images/no-image.png');
  } else {
    if($img_url <> '') {
...
function veronika_get_term($term = '', $country = '', $region = '', $city = ''){
  if( $term == '') {
    if( $city <> '' && is_numeric($city) ) {
      ...
      $loc = array_filter(array(osc_location_native_name_selector($city_info, 's_name'), osc_location_native_name_selector($region_info, 's_name'), strtoupper(@$region_info['fk_c_country_code'])));
      return implode(', ', $loc);
    }
 
    if( $region <> '' && is_numeric($region) ) {
      $region_info = Region::newInstance()->findByPrimaryKey( $region );
      $loc = array_filter(array(osc_location_native_name_selector($region_info, 's_name'), strtoupper(@$region_info['fk_c_country_code'])));
      return implode(', ', $loc);
    }
...
function veronika_simple_category( $select = false ) {
  ...
  $root = isset($root['pk_i_id']) ? $root : array('pk_i_id' => Params::getParam('sCategory'), 's_name' => (isset($c_category['s_name']) ? $c_category['s_name'] : ''));
  ...
  if($reset) {
    ...
    if(Params::getParam('sRegion') <> '' or Params::getParam('cookieAction') == 'done'or Params::getParam('cookieActionMobile') == 'done') {
      if(is_numeric(Params::getParam('sRegion'))) {
        ...
        mb_set_cookie('veronika-sRegion', osc_location_native_name_selector($reg, 's_name')); 
        mb_set_cookie('veronika-sCity', ''); 
      } else {
    ...
    if(Params::getParam('sCity') <> '' or Params::getParam('cookieAction') == 'done' or Params::getParam('cookieActionMobile') == 'done') {
      if(is_numeric(Params::getParam('sCity'))) {
        ...
        mb_set_cookie('veronika-sRegion', osc_location_native_name_selector($reg, 's_name')); 
        mb_set_cookie('veronika-sCity', osc_location_native_name_selector($city, 's_name')); 
      } else {
...
function veronika_location_format($country = null, $region = null, $city = null) { 
  if($country <> '') {
    ...
    if($region <> '') {
      if($city <> '') {
        return $city . ' ' . __('in', 'veronika') . ' ' . $region . (osc_location_native_name_selector($country_full, 's_name') <> '' ? ' (' . osc_location_native_name_selector($country_full, 's_name') . ')' : '');
      } else {
        return $region . ' (' . osc_location_native_name_selector($country_full, 's_name') . ')';
      }
    } else { 
      if($city <> '') {
        return $city . ' ' . __('in', 'veronika') . ' ' . osc_location_native_name_selector($country_full, 's_name');
      } else {
...
function theme_veronika_actions_admin() {
  ...
  if( Params::getParam('veronika_general') == 'done' ) {
    ...
    $lazyload = Params::getParam('lazyload');
    $hide_phone_nonlogged = Params::getParam('hide_phone_nonlogged');
    $hide_form_nonlogged = Params::getParam('hide_form_nonlogged');
    ...
    osc_set_preference('lazyload', ($lazyload ? '1' : '0'), 'veronika_theme');
    osc_set_preference('hide_phone_nonlogged', ($hide_phone_nonlogged ? '1' : '0'), 'veronika_theme');
    osc_set_preference('hide_form_nonlogged', ($hide_form_nonlogged ? '1' : '0'), 'veronika_theme');
...
if( !function_exists('veronika_theme_install') ) {
  ...
  function veronika_theme_install() {
    ...
    osc_set_preference('lazyload', '0', 'veronika_theme');
    ...
    osc_set_preference('hide_phone_nonlogged', '0', 'veronika_theme');
    osc_set_preference('hide_form_nonlogged', '0', 'veronika_theme');
...
function veronika_get_cat_icon( $id, $string = false ) {
  ...
  if(osc_get_preference('cat_icons', 'veronika_theme') == 1) { 
    if(isset($category_extra['s_icon']) && $category_extra['s_icon'] <> '') {
      ...
      if(isset($category['pk_i_id']) && isset($default_icons[$category['pk_i_id']]) && $default_icons[$category['pk_i_id']] <> '') {
...
function veronika_admin_menu_fix(){
  echo '<style>' . PHP_EOL;
  echo 'body.compact #veronika_menu .ico-veronika_menu {bottom:-6px!important;width:50px!important;height:50px!important;margin:0!important;background:#fff url(https://www.veronika.mb-themes.com/oc-content/themes/veronika/images/favicons/favicon-32x32.png) no-repeat center center !important;}' . PHP_EOL;
  ...
  echo 'body:not(.compact) #veronika_menu .ico-veronika_menu {background:transparent url(https://www.veronika.mb-themes.com/oc-content/themes/veronika/images/favicons/favicon-32x32.png) no-repeat center center !important;}' . PHP_EOL;
  echo '</style>' . PHP_EOL;
}
