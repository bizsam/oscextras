...
<head>
  ...
  <?php
    $location_array = array(osc_item_country(), osc_item_region(), osc_item_city(), osc_item_city_area());
  ...
  <?php 
    ...
    if(osc_item_show_phone() == 0) {
      ...
      $mobile = __('Login to see phone number', 'veronika');
      $mobile_found = true;
      $mobile_login_required = true;
    } else if(osc_get_preference('hide_phone_nonlogged', 'veronika_theme') == 1 && !osc_is_web_user_logged_in() && strlen(trim($mobile)) >= 4) {
      $mobile = __('Login to see phone number', 'veronika');
      $mobile_found = true;
      $mobile_login_required = true;
    ...
    if((osc_get_preference('hide_form_nonlogged', 'veronika_theme') == 1 && osc_is_web_user_logged_in()) || osc_get_preference('hide_form_nonlogged', 'veronika_theme') != 1) {
  ...
  <?php if($content_only == 0) { ?>
    ...
    <?php if(veronika_check_category_price(osc_item_category_id())) { ?><meta property="product:price:amount" content="<?php echo $og_price; ?>" /><?php } ?>
    <?php if(osc_item_price() <> '' && osc_item_price() <> 0 && veronika_check_category_price(osc_item_category_id())) { ?><meta property="product:price:currency" content="<?php echo osc_item_currency(); ?>" /><?php } ?>
  ...
  <div id="listing" class="content list">
              ...
              <div id="pictures" class="item-pictures">
                <ul class="item-bxslider">
                  ...
                  <?php for( $i = 0; osc_has_item_resources(); $i++ ) { ?>
                    <li>
                      <?php if($content_only == 0) { ?>
                        <a rel="image_group" data-fancybox="gallery" href="<?php echo osc_resource_url(); ?>" title="<?php echo osc_esc_html(osc_item_title()); ?> - <?php _e('Image', 'veronika'); ?> <?php echo $i+1;?>/<?php echo osc_count_item_resources();?>">
                      ...
                      <?php } else { ?>
                        <img data-fancybox="gallery" src="<?php echo osc_resource_url(); ?>" alt="<?php echo osc_esc_html(osc_item_title()); ?> - <?php echo $i+1;?>/<?php echo osc_count_item_resources();?>"/>
                      <?php } ?>
                    </li>
                  <?php } ?>
                </ul>
                  ...
                  <div id="item-bx-pager">
                    ...
                    <?php for( $i = 1; osc_has_item_resources(); $i++ ) { ?>
                      ...
                      <?php if($i - 1 + $at_once <= osc_count_item_resources()) { ?>
                        <a data-slide-index="<?php echo $c; ?>" data-fancybox="gallery2" href="" class="bx-navi<?php if($i - 1 + $at_once == osc_count_item_resources()) { ?> last<?php } ?>"<?php if($i - 1 + $at_once == osc_count_item_resources()) { ?> style="width:<?php echo $at_once * 128; ?>px"<?php } ?>>
                      <?php } ?>
                    ...
                    <div class="wrap">
                      <?php if($mobile_login_required) { ?>
                        <div class="phone<?php if(!$mobile_found) { ?> no-number<?php } ?>">
                          <a href="<?php echo osc_user_login_url(); ?>" class="phone-block login-required" data-item-id="<?php echo osc_item_id(); ?>" data-item-user-id="<?php echo osc_item_user_id(); ?>">
                            <i class="fa fa-phone"></i>
                            <span>
                              <?php echo __('Login to show number', 'veronika'); ?>
                            </span>
                          </a>
                        </div>
                      <?php } else { ?>
    ...
    <?php if($content_only == 0) { ?>
      <div id="side-right">
        ...
        <?php if(veronika_check_category_price(osc_item_category_id())) { ?>
        ...
        <?php if( !osc_item_is_expired() && (osc_logged_user_id() <> osc_item_user_id() || osc_logged_user_id() == 0 ) && ( !osc_reg_user_can_contact() || osc_is_web_user_logged_in() )) { ?>
          <div class="contact-button btn btn-primary <?php if(!$contact_form_shown) { ?>btn-disabled contact-disabled<?php } ?> tr1 round3 i-shadow<?php if($mobile_found) { ?> mobile-found<?php } ?>" <?php if(!$contact_form_shown) { ?>disabled onclick="return false;" title="<?php echo osc_esc_html(__('Login to contact seller', 'veronika')); ?>"<?php } ?>>
            <i class="fa fa-envelope"></i> 
            <span class="top"><?php _e('Contact seller', 'veronika') ; ?></span>

            <?php if($mobile_login_required) { ?>
              <span class="bot login-required"><?php echo __('Login to show number', 'veronika'); ?></span>
            <?php } else if($mobile_found && strlen($mobile) >= 4) { ?>
              <span class="bot"><?php echo substr($mobile, 0, strlen($mobile) - 4) . 'xxxx'; ?></span>
            <?php } else if(!$contact_form_shown) { ?>
              <span class="bot login-required"><?php echo __('Login to contact seller', 'veronika'); ?></span>
            <?php } ?>
          </div>
        <?php } ?>
  ...
  <?php if($content_only == 0) { ?>
    <script type="text/javascript">
      $(document).ready(function(){
        ...
        <?php if($mobile <> __('No phone number', 'veronika') && !$mobile_login_required) { ?>  
