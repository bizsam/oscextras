...
Version: 1.0  
File Description :This page will allow to admin to Edit the package.
*/
$conn = getConnection();

if(Params::getParam('editpkgid')){

	$editpkgid = Params::getParam('editpkgid');
	...
	{	  
	   ...
	   $packageIfo = $conn->osc_dbFetchResult("select * from %st_packages where package_id=%d",DB_TABLE_PREFIX,$editpkgid);

	   //print_r($packageIfo); 

	 }else{

	   osc_add_flash_warning_message( _m('Oops! something went wrong.'));
...
if($_SERVER['REQUEST_METHOD']=='POST' && Params::getParam('submit'))

   {
      ...
      $extratct = extract($_POST);
	   $exist = $conn->osc_dbFetchResults("SELECT * FROM %st_packages where package_name='%s' AND package_id!=%d AND status='1'",DB_TABLE_PREFIX,$package_name,$editpkgid);
	   $reCount = $conn->get_affected_rows();
	  if($reCount >0){
	   osc_add_flash_warning_message( _m('This package name is already exist.')) ;
	  }else{	   

	  $conn->osc_dbExec("UPDATE %st_packages set package_name='%s', package_description='%s',package_cost=%d,currency_code='%s', post_allow=%d, expiry_days='%s', period_type='%s', status='%d' where package_id=%d",DB_TABLE_PREFIX, $package_name,$package_description,$package_cost,$currency_code,$package_post_allow,$package_expiry_date,$period_type,$status,$editpkgid);

	  osc_add_flash_ok_message( _m('Data has been updated successfully') ) ;
...
<form action="" method="post">
  ...
  <div class="form-horizontal">
    ...
    <h3><?php echo __('Edit Package','subscription');?>:</h3>
    <div class="form-rowdiv">
      <div class="form-label"><?php echo __('Package Name','subscription');?> *</div>
      <div class="form-controls">
        <input type="text" class="xlarge" name="package_name" value="<?php echo $packageIfo['package_name']?>">
      </div>
    </div>
    <br />
    <div class="form-rowdiv">
      ...
      <div class="form-controls">
        <input type="text" class="xlarge" name="package_cost" value="<?php echo $packageIfo['package_cost']?>" maxlength="5">
		<select name="currency_code">
		   <?php foreach((array)osc_get_currencies() as $currency){?>
			<option value="<?php echo $currency['pk_c_code'].':'.$currency['s_description'];?>" <?php echo ($packageIfo['currency_code']==$currency['s_description'])?'selected="selected"':'';?>><?php echo $currency['s_description'];?></option>
		   <?php }?>
		</select>
    ...
    <div class="form-rowdiv">
      ...
      <div class="form-controls">
        <input type="text" class="xlarge" name="package_post_allow" value="<?php echo $packageIfo['post_allow']?>" maxlength="5">
      </div>
    </div>
    <br />
    <div class="form-rowdiv">
      ...
      <div class="form-controls">
        <input type="text" class="xlarge" name="package_expiry_date" value="<?php echo $packageIfo['expiry_days']?>" id="package_expiry_date1" />
		 <select name="period_type">
			<option value="days" <?php echo ($packageIfo['period_type']=='days')?'selected="selected"':'';?>>Days</option>
			<option value="month" <?php echo ($packageIfo['period_type']=='month')?'selected="selected"':'';?>>Month</option>
			<option value="year" <?php echo ($packageIfo['period_type']=='year')?'selected="selected"':'';?>>Year</option>
		 </select>
        </div>
    </div>
    <br />
    <div class="form-rowdiv input-description-wide">
      ...
      <div class="form-controls">
        <textarea name="package_description" rows="5" class="mceEditor"><?php echo $packageIfo['package_description']?></textarea>
      </div>
    </div>
    <br />
    <div class="form-rowdiv">
      ...
      <div class="form-controls">
        <input type="radio" class="xlarge" name="status" value="1" <?php echo ($packageIfo['status']==1)?'checked="checked"':'';?>>
        <?php echo __('Active','subscription');?>
        <input type="radio" class="xlarge" name="status" value="0" <?php echo ($packageIfo['status']==0)?'checked="checked"':'';?>>
        <?php echo __('Inactive','subscription');?> </div>
    </div>
    <br />
    <div class="form-actions">
      <input type="submit" value="<?php echo __('Update Package','subscription');?>" name="submit" class="btn btn-submit">
    </div>
  </div>
  </fieldset>
</form>